...
/**
 * Fury {@link Serializer} for jdkgdxds {@link ShortDeque}s.
 */
public class ShortDequeSerializer extends Serializer<ShortDeque> {

    public ShortDequeSerializer(Fury fury) {
        super(fury, ShortDeque.class);
    }

    @Override
    public void write(final MemoryBuffer output, final ShortDeque data) {
        output.writePrimitiveArrayWithSizeEmbedded(data.toArray(), Platform.SHORT_ARRAY_OFFSET, data.size() << 1);
    }

    @Override
    public ShortDeque read(MemoryBuffer input) {
        return new ShortDeque(Support.readShortsWithSizeEmbedded(input));
    }
}
...
/**
 * Fury {@link Serializer} for jdkgdxds {@link BooleanDeque}s.
 */
public class BooleanDequeSerializer extends Serializer<BooleanDeque> {

    public BooleanDequeSerializer(Fury fury) {
        super(fury, BooleanDeque.class);
    }

    @Override
    public void write(final MemoryBuffer output, final BooleanDeque data) {
        output.writePrimitiveArrayWithSizeEmbedded(data.toArray(), Platform.BOOLEAN_ARRAY_OFFSET, data.size());
    }

    @Override
    public BooleanDeque read(MemoryBuffer input) {
        return new BooleanDeque(Support.readBooleansWithSizeEmbedded(input));
    }
}